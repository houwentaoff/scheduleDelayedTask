...
//      printf("==>%s()head[0x%x]\n", __func__, head(fDelayQueue));
  ...
  while (timeVal.eq(&fDelayQueue->head(fDelayQueue)->fDeltaTimeRemaining.fTv, &fTv))
  {
	...
	/*
	tmp = [&]()->struct AlarmHandler *{
		const decltype(((struct AlarmHandler *)0)->fDelayQueueEntry)* __mptr = toRemove;    

		return (struct AlarmHandler *)((char *)__mptr - offsetof(struct AlarmHandler, fDelayQueueEntry)); 
	}();
	*/
